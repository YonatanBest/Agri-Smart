...
async def fertilizer_agent(
    ...
    crop_type: str = Query(..., description="Type of crop planted (e.g., maize, wheat)"),
    dt: str = Query(None, description="Current datetime in ISO format (optional, defaults to now)")
):
    try:
        if dt:
